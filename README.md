...
1. [php demo](https://github.com/jufengpay/jufengpay.github.io/tree/master/php-demo)
2. [java demo](https://github.com/jufengpay/jufengpay.github.io/tree/master/java-demo)
3. [c# demo](https://github.com/jufengpay/jufengpay.github.io/tree/master/C%23-demo)

### 同步通知 （returnUrl）
当创建订单时传入返回地址，订单结束后，用户点击“返回商户”，会在返回链接带上参数（returnUrl?urlparams）。参数内容[参考统一返回参数](https://jufengpay.github.io/#%E7%BB%9F%E4%B8%80%E8%BF%94%E5%9B%9E%E5%8F%82%E6%95%B0)，可通过签名算法计算签名的正确性。例：
```
returnUrl?
    amount=100&
    payMode=100001&
...
当创建订单时传入异步回调地址时，订单结束后（用户取消订单(-30)、用户支付超时（-40）、订单失败（-50）、订单已完成（50））进行通知，总共通知3次，间格时间分别为0s,15,60s，超时时间为10s，处理成功后返回 *success*，返回其他字符表示处理失败，会继续进行后续通知。通知内容[参考统一返回参数](https://jufengpay.github.io/#%E7%BB%9F%E4%B8%80%E8%BF%94%E5%9B%9E%E5%8F%82%E6%95%B0)，可通过签名算法计算签名的正确性
示例：
```
curl -X POST "回调地址"
...
2. 查询订单接口
    ...
    6. 响应（[参考统一返回参数](https://jufengpay.github.io/#%E7%BB%9F%E4%B8%80%E8%BF%94%E5%9B%9E%E5%8F%82%E6%95%B0)）
    5. 示例
    ```
...
1. 银行卡下发
    ...
    4. 请求参数
        ...
        |ts|是|整数|1575948756| 商户订单时间戳（秒级）|
        |remark|否|字符串(<100)|无| 备注 |
        |sign|是|字符串|$2a$10$JwOX9nmVHrE6o8vcoSmyd.T6...| 参数签名，请按照签名算法生成|
    
    6. 响应:参考[响应内容](https://jufengpay.github.io/#%E5%93%8D%E5%BA%94%E5%86%85%E5%AE%B9)
    5. 示例
    ```
